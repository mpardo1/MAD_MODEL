...
setwd(Path)
system("R CMD SHLIB model_1000eq.c")
dyn.load("model_1000eq.so")
...
out <- ode(Y, times, func = "derivs",
           parms = parms, dllname = "model_1000eq",
...
saveRDS(ode, file = "~/MAD_MODEL/SUR_MODEL/Code/ode_pseudo_1000eq.rds")
...
{ 
  ...
  if(x[1] < 0 | x[2] < 0 | x[3] < 0 ){
    ...
    population <- matrix(0,nrow = 1,ncol=1000)
    
    forcs_mat <- list(data.matrix(forcings))
    
    z <- ode(y = population,
             times = 0:nrow(y), func = "derivs", method = "ode45",
             dllname = "model_1000eq", initfunc = "initmod", nout = 0, 
...
ob_data <- readRDS(file = "~/MAD_MODEL/SUR_MODEL/Code/ode_pseudo_1000eq.rds")
l <- nrow(ob_data)
# ob_data[,2:1001] <- ob_data[,2:1001] + matrix(rnorm(l*1000,0,trueSD), ncol = 1000, nrow = l)
